package matchmaker.backend.constants;

/** Autocompletion for the permission codes :) */
public class Perm {
  public static final String CHALLENGE_READ = "CHALLENGE_READ";
  public static final String CHALLENGE_MANAGE = "CHALLENGE_MANAGE";
  public static final String DEPARTMENT_CREATE = "DEPARTMENT_CREATE";
  public static final String COMPANY_EDIT = "COMPANY_EDIT";
  public static final String COMPANY_GRADE = "COMPANY_GRADE";
  public static final String COMPANY_MANAGE = "COMPANY_MANAGE";
  public static final String DEPARTMENT_MANAGE = "DEPARTMENT_MANAGE";
  public static final String COMPANY_GRAPH_READ = "COMPANY_GRAPH_READ";
  public static final String MATCHMAKER_GRAPH_READ = "MATCHMAKER_GRAPH_READ";
}
